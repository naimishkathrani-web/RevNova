...
Write-Host "Found $($files.Count) day files to check" -ForegroundColor Cyan
...
foreach ($file in $files) {
    ...
    if ($content -match '<style>body\{font-family') {
        Write-Host "Updating $($file.Name)..." -ForegroundColor Yellow
        
        # Find the style tag and replace it
        $content = $content -replace '<style>body\{font-family.*?</style>', @'
    <style>
...
Write-Host ""
Write-Host "Complete! Updated $updatedCount files" -ForegroundColor Green
